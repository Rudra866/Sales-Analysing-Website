...
function TotalRevenueCard({totalRevenue, footer, header, percentage}: {date?: DateRange,
    totalRevenue:number, header: string, footer: string, percentage: number}) {
    return (
        <Card>
            ...
            <CardContent>
                ...
                <p className={"text-xs text-muted-foreground"}>
                    <span className={cn('text-[#adfa1d]')}>{(percentage*100).toFixed(1)}% </span>
                    {footer}
                </p>
            </CardContent>
        </Card>
...
function DashboardOverview() {
    const {data, date, setDate} = useDashboard()
    const [totalRevenue, setTotalRevenue] = useState<number>(0)
    // todo calculate total revenue for ranges
    useEffect(() => {
        setTotalRevenue(data
            ?.map((sale) => sale?.Total)
            .reduce((a, b) => a + b, 0) ?? 0)
        ...
        <>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                ...
                <TotalRevenueCard
                    ...
                    percentage={0.201}
                    header={`Total Revenue for the month`}
                    footer={`from last ${date?.to?.toDateString()}`}/>
                {/* todo */}
                <TotalRevenueCard
                    date={date}
                    totalRevenue={totalRevenue}
                    header={`Total Revenue for the year`}
                    percentage={1.345}
                    footer={`from last ${date?.to?.toDateString()}`}/>
                {/*<Card>*/}
                {/*    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">*/}

...
export default function Page() {
    ...
    function handleOnClick() {
        getCustomer(supabase, "Eleonor Naudet").then(r => {
            if (r?.id == 1) {
                console.log("Customer get successful.", r)
            } else {
                console.log("Customer get unsuccessful.")
            }
        }).catch(error => {
            console.log("Error occurred in customer get.")
        })
        getEmployee(supabase, "44351").then(r => {
            if (r?.id == 6) {
                console.log("Employee get successful.", r)
            } else {
                console.log("Employee get unsuccessful.")
            }
        }).catch(error => {
            console.log("Error occurred in employee get.")
        })
        getFinancing(supabase, "Dealer").then(r => {
            if (r?.id == 1) {
                console.log("Financing get successful.", r)
            } else {
                console.log("Financing get unsuccessful.")
            }
        }).catch(error => {
            console.log("Error occurred in financing get.")
        })
        getRole(supabase, 'Administrator').then(r => {
            if (r?.id == 1) {
                console.log("Role get successful.", r)
            } else {
                console.log("Role get unsuccessful.")
            }
        }).catch(error => {
            console.log("Error occurred in role get.")
        })
        getNotification(supabase, 1).then(r => {
            if (r?.id == 1) {
                console.log("Notification get successful.", r)
            } else {
                console.log("Notification get unsuccessful.")
            }
        }).catch(error => {
            console.log("Error occurred in notification get.")
        })
        getSale(supabase, '13365').then(r => {
            if (r?.id == 1) {
                console.log("Sale get successful.", r)
            } else {
                console.log("Sale get unsuccessful.")
            }
        }).catch(error => {
            console.log("Error occurred in sale get.")
        })
        getSalesGoal(supabase, 'goal 1').then(r => {
            if (r?.id == 1) {
                console.log("Sale goal get successful.", r)
            } else {
                console.log("Sale goal get unsuccessful.")
            }
        }).catch(error => {
            console.log("Error occurred in sale goal get.")
        })
        getTask(supabase, 'task 1').then(r => {
            if (r?.id == 1) {
                console.log("Task get successful.", r)
            } else {
                console.log("Task get unsuccessful.")
            }
        }).catch(error => {
            console.log("Error occurred in task get.")
        })
        getTradeIn(supabase, 'GMC Canyon 4WD').then(r => {
            if (r?.id == 1) {
                console.log("Trade in get successful.", r)
            } else {
                console.log("Trade in get unsuccessful.")
            }
        }).catch(error => {
            console.log("Error occurred in trade in get.")
        })
    ...
    return (
        <>
            <Button onClick={event => {
                handleOnClick()
            }}>Press to post shenanigans</Button>
        </>
    )
}

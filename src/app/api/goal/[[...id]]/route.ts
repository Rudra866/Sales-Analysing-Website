...
import {
  Database, getAllSalesGoals,
  getEmployeeFromAuthUser,
  getRoleFromEmployee,
  postToSalesGoals,
  SalesGoalInsert, SalesGoalUpdate
} from "@/lib/database";
...
export async function PATCH(request: Request, { params }: {params: {id: string[]}}) {
  if (!params.id) {
    NextResponse.json({error: "No goal ID provided to modify."}, {status: 400})
  }
...
export async function DELETE(request: Request, { params }: {params: {id: string[]}}) {
  if (!params.id) {
    NextResponse.json({error: "No goal ID provided to delete."}, {status: 400})
  }
  if (params.id.length > 1) {

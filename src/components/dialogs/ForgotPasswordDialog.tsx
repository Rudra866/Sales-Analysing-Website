...
export default function ForgotPasswordDialog({error, setError, success, setSuccess}: ForgotPasswordDialogProps) {
  ...
  function onSubmit(data: z.infer<typeof forgotPasswordDialogSchema>) {
    if (isButtonActive) {
      ...
      setError(`Please wait ${waitDuration/1000} seconds before trying again.`)
      setSuccess(false)
    }
  }
          ...
          <DialogBody>
            ...
            {success &&
                <Alert variant={"success"} className={"my-4"}>
                    <AlertDescription>
                        A password reset link was sent to the email address if an account exists!
                    </AlertDescription>
                </Alert>}
            {error &&

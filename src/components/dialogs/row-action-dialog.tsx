...
import { saleFormSchema } from "@/lib/zod-schemas";
...
export function RowActionDialog({ sale }: SaleSelectModalFormProps) {
  ...
  const [newSalesCheck, setNewSalesCheck] = useState<boolean>(
    sale?.NewSale || false,
  );
  ...
  // these fields with money values should be input inputType because of decimals.
  ...
  return (
    <ScrollArea className="flex flex-col max-h-[700px] text-foreground pr-4 ">
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit((data) => formContext?.onSubmit(data))}
        >
          <DialogBody className={"grid grid-cols-4 gap-2"}>
            {formFieldComponentsProps.map(
              (props, index) =>
                !props.hidden && (
                  <FormFieldComponent
